...
final class OSCMessage_ValueProtocol_Masks_Tests: XCTestCase {
    ...
    func testValuesFromValueMask1() throws {
        ...
        // success
        let result1 = try [OSCMessage.Value]([.int32(123)])
            .masked(mask)
        XCTAssertEqual(result1.count, 1)
        XCTAssertEqual(result1[0] as? Int32, 123)
        
        // fail - empty values array
        let result2 = try? [OSCMessage.Value]([])
            .masked(mask)
        XCTAssertNil(result2)
        
        // fail - wrong type
        let result3 = try? [OSCMessage.Value]([.int64(123)])
            .masked(mask)
        XCTAssertNil(result3)
        
        // fail - matches but too many values
        let result4 = try? [OSCMessage.Value]([.int32(123), .int32(123)])
            .masked(mask)
