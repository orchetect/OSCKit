...
final class DataExtensions_Tests: XCTestCase {
    ...
    func testExtractInt32() {
        ...
        XCTAssertEqual(data1.extractInt32()?.byteLength, 4)
    ...
    func testExtractInt64() {
        ...
        XCTAssertEqual(data1.extractInt64()?.byteLength, 8)
    ...
    func testExtractFloat32() {
        ...
        XCTAssertEqual(data1.extractFloat32()?.byteLength, 4)
    ...
    func testExtractDouble() {
        ...
        XCTAssertEqual(data1.extractDouble()?.byteLength, 8)
    ...
    func testExtractNull4ByteTerminatedString() {
        ...
        XCTAssertEqual(data1.extractNull4ByteTerminatedASCIIString()?.byteLength, 4)
    ...
    func testExtractNull4ByteTerminatedData() {
        ...
        XCTAssertEqual(data1.extractNull4ByteTerminatedData()?.byteLength, 4)
        
        // string
        
        let data2 = Data([0x53, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x00, 0x00]) // "String" null null
        XCTAssertEqual(
            data2.extractNull4ByteTerminatedData()?.data,
            Data([0x53, 0x74, 0x72, 0x69, 0x6E, 0x67])
        )
        XCTAssertEqual(data2.extractNull4ByteTerminatedData()?.byteLength, 8)
    ...
    func testExtractBytesCount() {
        ...
        // not null padded to multiples of 4 bytes
        XCTAssertNil(data.extract(byteLength: -1))
        XCTAssertEqual(data.extract(byteLength: 0), Data([]))
        XCTAssertEqual(data.extract(byteLength: 1), Data([0x01]))
        XCTAssertEqual(data.extract(byteLength: 2), Data([0x01, 0x02]))
        XCTAssertEqual(data.extract(byteLength: 3), Data([0x01, 0x02, 0x03]))
        XCTAssertEqual(data.extract(byteLength: 4), Data([0x01, 0x02, 0x03, 0x04]))
        XCTAssertNil(data.extract(byteLength: 5))
        XCTAssertNil(data.extract(byteLength: 6))
    }
}
