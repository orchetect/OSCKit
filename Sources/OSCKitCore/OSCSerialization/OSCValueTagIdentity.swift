...
public enum OSCValueTagIdentity: Equatable, Hashable {
    ...
    /// Variable:
    /// The OSC value occupies a single static OSC-type tag character but it varies depending on the content of the value.
    /// All possible type tags must be finite and known at compile time.
    ///
    /// An example is Boolean (true/false). A single instance of the concrete type occupies a single tag but the tag may be "t" or "f".
    case variable([Character])
    
    /// Variadic:
    /// The OSC "value" may be complex in nature and occupies one or more OSC-type tag characters and they are not reasonably known at compile time. This scenario requires manual parsing and validation. Very few implementations will require this pattern.
    ///
    /// One (perhaps the only) example of a variadic implementation is an OSC value array. It starts with the "[" tag and ends with the "]" tag and may contain zero or more atomic values. It would have a `minCount` of 2 and a `maxCount` of `nil`. Variadic is how OSCKit itself internally implements OSC array encoding and decoding.
